...
/** True if `role` holds every one of `capabilities`. */
export function canAll(role: string | null | undefined, capabilities: Capability[]): boolean {
  return capabilities.every(c => can(role, c))
}

/** True if `role` holds at least one of `capabilities`. */
export function canAny(role: string | null | undefined, capabilities: Capability[]): boolean {
  return capabilities.some(c => can(role, c))
}

/**
 * Every capability a role holds — for handing the client a set it can check
 * without shipping the mapping table to the browser.
 */
export function capabilitiesFor(role: string | null | undefined): Capability[] {
  if (!role) return []
  const granted = ROLE_CAPABILITIES[role]
  if (!granted) return []
  if (granted[0] === ALL) return [...CAPABILITIES]
  return [...(granted as readonly Capability[])]
}

/**
 * Roles that hold a capability. Used where a query needs to *find* the people
 * who can do something — e.g. picking a fallback approver, or deciding who to
 * notify that a work pass is expiring.
 */
export function rolesWith(capability: Capability): string[] {
  return Object.keys(ROLE_CAPABILITIES).filter(role => can(role, capability))
}

/**
 * Convenience for the many places that mean "HR or better". Prefer a specific
 * capability where one fits — this exists for genuinely broad checks like
 * "should this person see the HR side of the documents browser".
 */
export function isHrOrAdmin(role: string | null | undefined): boolean {
